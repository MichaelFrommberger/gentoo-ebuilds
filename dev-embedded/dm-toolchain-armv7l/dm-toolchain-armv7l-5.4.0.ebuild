...
DESCRIPTION="""Cross-compile toolchain for arm microcontroller from
			Diehl Metering Deutschland GmbH."""
...
src_unpack() {
	...
	cp -v "${FILESDIR}"/dm-config-armv7l-${PV} ${S}/.config || die
...
src_install() {
	local inst_dir=$(sed -n "s:CT_PREFIX_DIR=::p" ${FILESDIR}/dm-config-armv7l-${PV} | sed 's:["",]::g')

	# cp from temporary install dir to "real" install dir 
	cp -R ${T}/install/* "${D}" || die "Install failed!"

	echo -n "PATH=${inst_dir}/bin" > "${T}/99dm-toolchain-armv7l"
	doenvd "${T}/99dm-toolchain-armv7l"
}
