...
DESCRIPTION="Cross-compile toolchain for PC (x86 32 bit) from Diehl Metering Detschland GmbH."
...
src_unpack() {
	...
	cp -v "${FILESDIR}"/dm-config-x86_32-${PV} ${S}/.config || die
...
src_install() {
	local inst_dir=$(sed -n "s:CT_PREFIX_DIR=::p" ${FILESDIR}/dm-config-x86_32-${PV} | sed 's:["",]::g')

	# cp from temporary install dir to "real" install dir 
	cp -R ${T}/install/* "${D}" || die "Install failed!"

	echo -n "PATH=${inst_dir}/bin" > "${T}/99dm-toolchain-x86_32"
	doenvd "${T}/99dm-toolchain-x86_32"
}
